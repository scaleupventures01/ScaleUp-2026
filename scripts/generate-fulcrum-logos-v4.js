      ...
      res.on('end', async () => {
        try {
          ...
          if (response.error) {
            ...
            if (response.error.message.includes('quota') && retryCount < 5) {
              ...
              await new Promise(r => setTimeout(r, waitTime + 2000)); // Add 2s buffer
              try {
          ...
          if (response.candidates && response.candidates[0]) {
            ...
            // If text response found, show it
            for (const part of parts) {
              if (part.text) {
                console.log(`  Model response text: ${part.text.substring(0, 200)}`);
              }
            }
...
async function main() {
  ...
  console.log('With automatic rate limit handling (up to 5 retries)');
