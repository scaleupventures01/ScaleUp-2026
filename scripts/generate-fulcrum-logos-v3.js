...
// Try using Imagen 3 with the correct predict endpoint
    ...
    const req = https.request(apiUrl, options, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          console.log('Imagen response:', JSON.stringify(response, null, 2).substring(0, 1000));

          if (response.error) {
            reject(new Error(`Imagen API Error: ${response.error.message}`));
            return;
          }
          ...
          if (response.predictions && response.predictions[0]) {
            ...
            if (imageData) {
              ...
              resolve({ success: true, path: outputPath, model: 'imagen-3.0-generate-001' });
              return;
            }
          }

          reject(new Error('No image data found in Imagen response'));
        } catch (e) {
...
// Use gemini-2.0-flash-exp with rate limit retry
async function generateWithGemini(prompt, outputPath, retryCount = 0) {
      ...
      res.on('end', async () => {
        try {
          ...
          if (response.error) {
            ...
            if (response.error.message.includes('quota') && retryCount < 3) {
              const waitMatch = response.error.message.match(/retry in ([\d.]+)s/);
              const waitTime = waitMatch ? parseFloat(waitMatch[1]) * 1000 : 60000;
              console.log(`    Rate limited. Waiting ${Math.ceil(waitTime / 1000)}s before retry ${retryCount + 1}/3...`);
              await new Promise(r => setTimeout(r, waitTime + 5000)); // Add 5s buffer
              try {
                const result = await generateWithGemini(prompt, outputPath, retryCount + 1);
                resolve(result);
              } catch (retryError) {
                reject(retryError);
              }
          ...
          if (response.candidates && response.candidates[0]) {
            ...
            for (const part of parts) {
              if (part.inlineData && part.inlineData.mimeType.startsWith('image/')) {
                ...
                fs.writeFileSync(outputPath, imageBuffer);
                console.log(`Saved image to: ${outputPath}`);
                resolve({ success: true, path: outputPath, model: 'gemini-2.0-flash-exp' });
                return;
              }
            }

            console.log('Response parts (no image):', JSON.stringify(parts, null, 2).substring(0, 500));
            reject(new Error('No image data found in Gemini response'));
          } else {
            console.log('Unexpected response:', JSON.stringify(response, null, 2).substring(0, 500));
...
async function main() {
  ...
  console.log('='.repeat(60));
  console.log('');
  console.log('Using gemini-2.0-flash-exp with automatic rate limit handling');
  ...
  for (let i = 0; i < prompts.length; i++) {
    ...
    try {
      ...
      console.error(`  Gemini failed: ${error.message.substring(0, 100)}`);

      // Fall back to Imagen
      try {
        console.log('  Trying Imagen 3...');
        const result = await generateWithImagen(prompt, outputPath);
        results.push({
          name,
          title,
          status: 'success',
        ...
        console.error(`  Imagen failed: ${imagenError.message.substring(0, 100)}`);
        results.push({
          name,
          title,
          status: 'error',
          error: `Gemini: ${error.message} | Imagen: ${imagenError.message}`,
          prompt: prompt.substring(0, 500) + '...'
        });
      }
    }
